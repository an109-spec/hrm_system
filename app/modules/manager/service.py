...
class ManagerService:
    ...
    def _latest_salary(employee_id: int) -> Salary | None:
        return Salary.query.filter_by(employee_id=employee_id, is_deleted=False).order_by(Salary.year.desc(), Salary.month.desc()).first()

    @staticmethod
    def get_department_employee_summary(manager_id: int) -> dict:
        employees = ManagerService._get_subordinates(manager_id)
        today = parse_simulated_time({}).date()
        today_leave_ids = ManagerService._approved_leave_employee_ids([x.id for x in employees], today)
        near_contract_end = 0
        attendance_alert = 0
        probation_count = 0
        active_count = 0
        for emp in employees:
            ...
            if latest_contract and latest_contract.end_date and 0 <= (latest_contract.end_date - today).days <= 30:
                near_contract_end += 1
            alert_count = Attendance.query.filter(
        ...
        return {
            ...
            "on_leave_today": len(today_leave_ids),
            "expiring_contracts": near_contract_end,
            "attendance_alerts": attendance_alert,
        }
    ...
    def get_department_employee_list(manager_id: int, filters: dict | None = None) -> list[dict]:
        filters = filters or {}
        employees = ManagerService._get_subordinates(manager_id)
        today = parse_simulated_time(filters).date()
